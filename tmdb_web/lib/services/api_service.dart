...
class APIService implements IAPIService {
  ...
  Future<TrendingMovie> fetchTrendingMovies() async {
    final response =
        await http.get('$baseUrl/trending/movie/day?api_key=$_apiKey');
    print(response.body);
    if (response.statusCode == 200) {
      final parsed =
          json.decode(response.body)['results'].cast<TrendingMovie>();
      print(parsed);
      return parsed.map((json) => TrendingMovie.fromJson(json)).toList();
    } else {
